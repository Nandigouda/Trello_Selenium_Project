...
public static void main(String[] args) throws Throwable {
	 ...
	 * FileInputStream("./src/test/resource/facebookdropdown1.xlsx"); Workbook
	 * workbook = WorkbookFactory.create(fis); Sheet sheet =
	 * workbook.getSheet("facebookdropdown1"); sheet.getSheetName();
	 * sheet.getTopRow(); sheet.getFitToPage(); //System.out.println("fit: "+fit);
	 * int firstRowIndex = sheet.getFirstRowNum();
	 * System.out.println("firstRowIndex: "+firstRowIndex); int lastRowIndex =
	 * sheet.getLastRowNum();
	 * 
	 * System.out.println("lastRowIndex: "+lastRowIndex); int noOfRows =
	 * sheet.getPhysicalNumberOfRows();
	 * 
	 * System.out.println("noOfRows: "+noOfRows);
	 * 
	 * 
	 * for(int i = firstRowIndex;i<=lastRowIndex; i++) { short firstCellIndex =
	 * sheet.getRow(i).getFirstCellNum();
	 * System.out.println("firstCellIndex: "+firstCellIndex); short cellCountInRow =
	 * sheet.getRow(i).getLastCellNum();
	 * System.out.println("cellCountInRow :"+cellCountInRow); for(short
	 * j=firstCellIndex; j<cellCountInRow;j++) { String cellType =
	 * sheet.getRow(i).getCell(j).getCellType().toString(); if
	 * (cellType.equals("NUMERIC")) { long numericValue =
	 * (long)sheet.getRow(i).getCell(j).getNumericCellValue(); if
	 * (cellCountInRow==31) { expectedDayList.add(String.valueOf(numericValue));
	 * }else if (cellCountInRow==119) {
	 * expectedYearList.add(String.valueOf(numericValue)); } }else if
	 * (cellType.equals("STRING")) { String stringValue =
	 * sheet.getRow(i).getCell(j).getStringCellValue();
	 * System.out.println("stringValue: "+stringValue);
	 * expectedMonthList.add(stringValue); } } } workbook.close();
	 */
